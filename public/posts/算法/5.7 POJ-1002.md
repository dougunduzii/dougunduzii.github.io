...
[http://poj.org/problem?id=1002&lang=zh-CN&change=true](http://poj.org/problem?id=1002&lang=zh-CN&change=true)
...
if not find:
    print('No duplicates.')
for key in sorted(timesOfTel):
    if timesOfTel[key] > 1:
        print(key,timesOfTel[key])
```
